...
2. Атомы тяжёлых галогенов $text("X")$ обладают высокой поляризуемостью, в некоторых галогенпроизводных $text("RX")$ электронное окружение атома галогена может быть смещено к заместителю $text("R")$, приводя к обнажению небольшого положительного заряда (т.н. $sigma$-дырки) на атоме галогена со стороны @Lu2012, противоположной связи $text("R-X")$. Этот положительный заряд может участвовать в электростатических взаимодействиях с другими атомами, образуя _галогеновые связи_ (по аналогии с водородными). $sigma$-Дырки могут быть описаны добавлением небольшого положительного заряда в нужною область пространство вблизи атома галогена.
...
3. Методом наименьших квадратов находятся оценки зарядов на атомах:
  ...
  - Ищется экстремум целевой функции, отвечающий минимуму:
    $
      (diff chi^2)/(diff hat(q)_j) = (diff chi^2_"esp")/(diff hat(q)_j) + (diff chi^2_"rstr")/(diff hat(q)_j) = 0 quad j = 1..N.
    $
...
3. СП семейства `CHARMM` имеют дополнительный терм --- т.н. потенциал Юри-Брэдли @namd-potential. Для описания напряжения, возникающего для тройки ковалентно связанных атомов $text("X-Y-Z")$, вводится 2 параболических потенциала: 1й, на величину валентного угла $theta = hat(text("X, Y, Z"))$, 2й на расстояние $r_t = |bold(r)_text("X") - bold(r)_text("Z")|$ между терминальными атомами в тройке. Таким образом, потенциал валентного угла
  $
...
4. Вид потенциала взаимодействия 2 ковалентно несвязанных атомов $text("X")$ и $text("Y")$ задаются #link("../2021-01-00-molecular-interactions/index.typ#U-LJ")[уравнением Ленарда --- Джонса]. Его можно переформулировать через параметры глубины потенциальной ямы (энергии взаимодействия) $epsilon_(text("X"), text("Y"))$ и соответствующего ей расстояния $r^("min")_(text("X"),text("Y"))$:
  $
    U_"LJ"(r) = epsilon_(text("X"), text("Y")) {((r^"min"_(text("X"), text("Y")))/(r))^12 - 2 ((r^"min"_(text("X"), text("Y")))/(r))^6}.
  $
  Если природа взаимодействующих атомов идентична ($text("X") = text("Y")$), то параметры $epsilon_(text("X"), text("X"))$ и $r^("min")_(text("X"), text("X"))$ берутся непосредственно из СП для соответствующего $text("X")$ типа атома, если $text("X") != text("Y")$, то параметры попарных взаимодействий необходимо вычислять. В силовых полях `OPLS-AA` и `GROMOS` они вычисляются как среднее геометрическое
  $
    epsilon_(text("X"),text("Y")) := sqrt(epsilon_(text("X"), text("X")) dot epsilon_(text("Y"),text("Y"))), quad r^("min")_(text("X"),text("Y")) := sqrt(r^("min")_(text("X"),text("X")) dot r^("min")_(text("Y"),text("Y"))),
  $
  в силовых полях `CHARMM` и `AMBER` аналогично вычисляется $epsilon_(text("X"),text("Y"))$, а $r^("min")_(text("X"),text("Y"))$ как среднее арифметическое:
  $
    r^("min")_(text("X"),text("Y")) = 1/2 (r^("min")_(text("X"),text("X")) + r^("min")_(text("Y"),text("Y"))).
  $
...
$
где $chi$ и $eta$ --- электроотрицательность ($diff E_p / diff q$) и жёсткость ($diff^2 E_p / diff q^2$) соответственно. Этот подход применим только для описания поляризации, возникающей вдоль химической связи, и, например, не способен описать перераспределение электронной плотности в $pi$-облаках ароматических колец.
