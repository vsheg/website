...
services:
  3x-ui:
    ...
    ports:
        ...
        - 127.0.0.1:8181:81 # subscription service # <4>
```
...
::: {.callout-tip}
Temporary use `network_mode: host` in the `compose.yaml` to test Xray without Caddy, it should already work with specified ports. See more in the [PS](#ps) section above.
:::
...
2. add gRPC-specific routing to your Caddy configuration as an ordinary path (`/secret/grpc*`) and explicitly set the transport to `h2c` (HTTP2 without encryption) used by gRPC.
...
### Transport option C: XHTTP with multitransport
...
::: {.callout-note}
WebSocket, gRPC, and XHTTP protocols operate over HTTP, so they can be routed through a CDN (e.g. Cloudflare, Fastly, Bunny). When routing Xray traffic through a CDN, your server’s IP remains hidden behind their network. However, your traffic is decrypted at the CDN edge, so it's not end-to-end encrypted or private. Also, a CDN may increase latency (ping), though it may also improve overall speeds by routing traffic through the optimized network.
:::
...
https://service.example.com {    
  ...
  reverse_proxy /secret/sub* http://localhost:8181 # <1>
  reverse_proxy /secret/json* http://localhost:8181 # <2>
}
```
1. Handles all requests to the 'ordinary' subscription service.
2. Handles all requests to the JSON subscription service.

You need to enable subscription service in 3X-UI panel settings, configure its listen IP (`0.0.0.0` in this set up), port (`8181` on host and `81` inside the container^[see `compose.yaml` for 3X-UI below]), and two paths[^subs] in order to generate correct subscription links automatically.
