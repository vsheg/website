...
&= \dfrac{1}{N! h^{3N}} \int_\text{ФП} \exp \left\{ - \beta \left[\sum_{i=1}^N \dfrac{\mathbf{p}_i^2}{2m_i}\right] + U_{\mathcal{A}} (\mathbf{q}_1, \cdots, \mathbf{q}_N)\right\} \mathrm{d} \mathbf{п} \mathrm{d} \mathbf{q},
\end{aligned}
$${#eq-def-Q}
...
В постановке задачи, состоянию $\mathcal A$ соответствует несвязанные белок s:{E} и лиганд s:{L}, состоянию $\mathcal B$ --- фермент-субстратный комплекс s:{EL}, на масштабах системы «лиганд---белок---гидратная оболочка» переход из несвязанного в связанное состояние в принципе можно считать небольшим возмущением.
